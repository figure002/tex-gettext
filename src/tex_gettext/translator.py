...
class Translation:
    ...
    def __init__(self, input, locale, file=None):
        ...
        self._icu_date_full = icu.DateFormat.createDateInstance(
            icu.DateFormat.FULL, self._icu_locale
        )

    def __repr__(self):
        return (
            f"Translation(input={self.input}, locale={self.locale}, file={self.file})"
        )
    ...
    def update(self, document):
        ...
        sys.stderr.write(f"Updating translation {self}...\n")
        if not os.path.exists(self.file):
            sys.stderr.write(f"Generating new translation file: {self.file}...\n")
            subprocess.check_call(
                ["msginit", "-i", template_name, "-l", self.locale, "-o", self.file]
            )
            return True
        with open(self.file, "rb") as f:
            old = f.read()
        sys.stderr.write(f"Merging template into translation file: {self.file}...\n")
    ...
    def translate(self, document):
        sys.stderr.write(f"Translating {document} to {self}...\n")
        ...
        with open(document.name) as input_file:
            doc = input_file.read()
            sys.stderr.write(f"Generating file {translated}...\n")
    ...
    def generate_template(self, document):
        with open(document.name) as doc:
            ...
            sys.stderr.write(f'Generating template "{template_name}"...\n')
            with open(template_name, "w") as template:
                ...
                for tag in tags:
                    ...
                    if tag.name == "\\gettext":
                        template.write(
                            f'{self.TAG_MSGID} "{escape(tag.args[0].content)}"\n'
                        )
                        template.write(f'{self.TAG_MSGSTR} ""\n')
                    elif tag.name == "\\ngettext":
                        template.write(
                            f'{self.TAG_MSGID} "{escape(tag.args[0].content)}"\n'
                        )
                        template.write(
                            f'{self.TAG_MSGID_PLURAL} "{escape(tag.args[1].content)}"\n'
                        )
                        template.write(f'{self.TAG_MSGSTR}[0] ""\n')
                        template.write(f'{self.TAG_MSGSTR}[1] ""\n')
                    elif tag.name == "\\pgettext":
                        template.write(
                            f'{self.TAG_MSGCTXT} "{escape(tag.args[0].content)}"\n'
                        )
                        template.write(
                            f'{self.TAG_MSGID} "{escape(tag.args[1].content)}"\n'
                        )
                        template.write(f'{self.TAG_MSGSTR} ""\n')
                    elif tag.name == "\\npgettext":
                        template.write(
                        ...
                        template.write(
                            f'{self.TAG_MSGID_PLURAL} "{escape(tag.args[2].content)}"\n'
                        )
                        template.write(f'{self.TAG_MSGSTR}[0] ""\n')
                        template.write(f'{self.TAG_MSGSTR}[1] ""\n')
                    template.write("\n")
        ...
        if tag.name == "\\gettext":
            ...
            if not self.file:
                ...
                variants = [
                    ...
                    if k.startswith(self.TAG_MSGSTR + "[")
                ]
            ...
            return self._icu_date_full.format(
            ...
            return self._icu_date_full.format(
                float(
                    datetime.datetime(
                        *[int(i.content) for i in tag.args][::-1]
                    ).timestamp()
                )
            )
        else:
    ...
    def _ensure_parsed(self):
        ...
        if self._parsed:
            return
        sys.stderr.write(f"Parsing {self.file}\n")
        with open(self.file) as f:
            ...
            next_tag_content: str | None = None
...
def find_translations(input_file, directory=None, languages=None):
    ...
    if languages:
        ...
        for i in languages:
            ...
            result.append(
                Translation.load(input_file, filename, Translation.ALLOW_NOT_EXISTING)
            )
    else:
...
def convert_plurals(description, n, variants):
    try:
        ...
        if not nplurals.startswith(NPLURALS):
            raise Exception(
                f'First element "{nplurals}" does not start with "{NPLURALS}"'
            )
        ...
        if not plural.startswith(PLURAL):
            raise Exception(f'Second element "{plural}" does not start with "{PLURAL}"')
        ...
        raise Exception(
            'Plurals definition must be formed as "nplurals: <n>; plural=<rule>"'
        ) from exc

    if len(variants) != nplurals:
        raise Exception(
            f"Invalid number of variants found (expected {nplurals}, "
            f"but {len(variants)} found)"
        )
